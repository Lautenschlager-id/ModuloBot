...
newParser = function()
	...
	function XmlParser:FromXmlString(value)
		...
		for c in value:gmatch("[^%s]+") do
			newStr[#newStr + 1] = c:gsub("&#x([%x]+);", function(h)
				return utf8frompoints(tonumber("0x" .. h))
			end)
		end
		value = table.concat(newStr, " ")
...
newNode = function(name)
	...
	function node:setValue(val) self.___value = tonumber(val) or val end
