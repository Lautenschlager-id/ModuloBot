	...
	f = function(message, parameters)
		...
		if parameters and #parameters > 0 then
			...
			for role in string.gmatch(parameters, "[^,]+") do
				...
				roles[len] = message.guild.roles:find(function(r)
					return string.lower(r.name) == role
				end)

				if not roles[len] then
					sendError(message, "LIST", "The role '" .. role .. "' does not exist.", syntax)
					return
				end
			end
			...
			toDelete[message.id] = message:reply({
				embed = {
					color = color.sys,
					title = "<:wheel:456198795768889344> Members with the role(s) " .. concat(roles, " - ", function(index, value) return string.upper(value.name) end) .. " ( " .. #members .. " )",
...
commands["profile"] = {
	...
	f = function(message, parameters)
		...
		if hasPermission(permissions.is_evt, p.discord) then
			icon = icon .. "<:idea:463505036564234270> "
		...
		if hasPermission(permissions.is_fc, p.discord) then
			icon = icon .. "<:fun:526827301107662858> "
...
commands["resetactivity"] = {
	...
	f = function(message, parameters)
		...
		messageCreate(client:getChannel("465525265985175552"):getMessage("530800717544423425"), true)
				...
				if hash == reactions.bug then
					local msg = client:getChannel(channels["bug-log"]):send({
						embed = {
							...
							image = (message.embed and (message.embed.image and { url = message.embed.image.url }) or nil)
						}
					})
	...
	elseif not skipChannelActivity and message.author.id ~= client.user.id then
		...
		if (os.time() - 10) < t then
			...
			client:getChannel(channels["chat-log"]):send({
				content = "Deleted by <@" .. message.author.id .. ">",
				embed = {
					description = message.content,
